...
import { Ionicons } from '@expo/vector-icons'; // Assuming Expo, using vector icons for visual flair
...
export default function LoginScreen({ navigation }) {
    ...
    const handleLogin = async () => {
        ...
        if (!login.fulfilled.match(resultAction)) {
            Alert.alert('Login Failed', resultAction.payload?.msg || 'Network error');
        }
    };
                ...
                <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 24, justifyContent: 'center' }}>
                    ...
                    <View className="items-center mb-10">
                        <View className="bg-[#EADDFF] p-4 rounded-3xl mb-4">
                            <Ionicons name="wallet-outline" size={32} color="#21005D" />
                        </View>
                        <Text className="text-4xl font-bold text-[#1D1B20] text-center">Welcome back</Text>
                        <Text className="text-[#49454F] text-base mt-2 text-center">Please enter your details to sign in.</Text>
                    </View>
                    ...
                    <View className="mt-4 items-end">
                        <TouchableOpacity><Text className="text-[#6750A4] font-bold">Forgot Password?</Text></TouchableOpacity>
                    </View>
                    ...
                    <TouchableOpacity
                        ...
                        disabled={isLoading}
                    >
                        {isLoading ? <ActivityIndicator color="#fff" /> : <Text className="text-white font-bold text-lg">Log In</Text>}
                    </TouchableOpacity>

                    <View className="flex-row justify-center mt-8">
                        <Text className="text-[#49454F] text-base">Don't have an account? </Text>
                        <TouchableOpacity onPress={() => navigation.navigate('Signup')}>
                            <Text className="text-[#6750A4] font-bold text-base">Sign Up</Text>
                        </TouchableOpacity>
                    </View>
